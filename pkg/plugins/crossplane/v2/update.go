...
and are overwritten; files without it (controller.go, *_types.go, go.mod, crossplane.yaml)
are never touched. The working tree must be clean; the result is left uncommitted so you
can review it with 'git diff' before committing.`,
...
func renderToMemFS(cfg config.Config, memFS machinery.Filesystem) error {
	...
	builders := make([]machinery.Builder, 0, len(initTemplates)+len(staticTemplates)+2)
	...
	for i := range resources {
		...
		apiBuilders := make([]machinery.Builder, 0, len(apiTemplates))
		for _, t := range apiTemplates {
			apiBuilders = append(apiBuilders, t)
		}
		if err := apiScaffold.Execute(apiBuilders...); err != nil {
