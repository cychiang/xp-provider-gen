...
var _ machinery.Template = &CrossplaneController{}

// CrossplaneController scaffolds the controller file for Crossplane managed resources
type CrossplaneController struct {
...
func (f *CrossplaneController) SetTemplateDefaults() error {
	if f.Path == "" {
		// Crossplane providers typically organize controllers by group
		if f.Resource.Group != "" {
			f.Path = filepath.Join("internal", "controller", "%[group]", "%[kind]", "%[kind].go")
		} else {
			// Fallback for resources without group
			f.Path = filepath.Join("internal", "controller", "%[kind]", "%[kind].go")
		}
	}
	...
	f.TemplateBody = crossplaneControllerTemplate
