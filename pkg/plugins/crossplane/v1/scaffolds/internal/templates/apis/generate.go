...
// Generate scaffolds the apis/generate.go file for code generation
...
func (f *Generate) SetTemplateDefaults() error {
	if f.Path == "" {
		f.Path = filepath.Join("apis", "generate.go")
	}
...
const generateTemplate = `// NOTE: See the below link for details on what is happening here.
