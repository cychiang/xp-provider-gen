...
config); leave it off user-edited files (`controller.go`, `*_types.go`) and seed-once files
(`go.mod`, `crossplane.yaml`, Makefile). See [architecture.md](architecture.md#6-ownership-contract-the-upgrade-foundation).

## Updating an existing provider

`xp-provider-gen update` refreshes a provider's tool-owned core (registration, `setup.go`
wiring, `main.go`, framework dependency versions) to the current generator, leaving user logic
and the diff for review:
...
`update` requires a clean tree and never commits. User files (`controller.go`, `*_types.go`)
and `go.mod`'s own requires are preserved; framework versions are bumped via `go get`.
