...
|--------|-------|-------------|
| Tool-owned (header) | `<kind>/wiring.go`, `internal/provider/connector.go`, all `register.go`, `config.go`, `main.go`, `doc.go`, `generate.go`, `groupversion_info.go`, `version.go`, `docs/ownership.md`, `hack/xp-provider-gen.mk`, `cluster/local/integration_tests.sh` | overwritten; removed if this render no longer produces it |
...
The generated Makefile is split along the same line: the user-owned `Makefile` holds only the
project variables and `include hack/xp-provider-gen.mk`; the tool-owned fragment holds the
build pipeline (submodule includes, generation, test/e2e/local-dev targets and, for upjet, the
Terraform CLI version and its download/schema logic), so pipeline fixes reach existing providers
on `update`.
...
4. **Bump dependencies** from the flavor's manifest set via `go get` (go.mod's own requires
   preserved); with `--terraform-provider-version` (upjet only — refused by flavor on a native
   provider, `tfversion.go`), also rewrites the pinned Terraform provider version.
...
to memfs → reconcile via the ownership gate (upjet: missing user-owned files are not seeded) →
remove orphaned tool-owned files this render no longer produces → bump the flavor's deps via
`go get` (plus `--terraform-provider-version`, upjet only) → finalize (native:
tidy/generate/reviewable; upjet: generate/tidy/reviewable) → stamp provenance (no commit; review
the diff).
