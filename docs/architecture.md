...
|--------|-------|-------------|
| Tool-owned (header) | `<kind>/wiring.go`, `internal/provider/connector.go`, all `register.go`, `config.go`, `main.go`, `doc.go`, `generate.go`, `groupversion_info.go`, `version.go`, `docs/ownership.md`, `hack/xp-provider-gen.mk` | overwritten; removed if this render no longer produces it |
