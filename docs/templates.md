...
2. **Decide ownership with the header.** Include the line
   `// Code generated by xp-provider-gen. DO NOT EDIT.` within the first 1024
   bytes (`headerScanLimit`) of the body (any comment syntax that keeps it
   literal) and the file is **tool-owned**:
...
Upjet bodies also see `.TerraformProvider`, `.TerraformProviderName`,
...
| `config/zz_resources.go` (upjet) | `generators/upjet_resources.go.tmpl` | `upjet_generator.go` |
| `test/behavior/<name>/chainsaw-test.yaml` (`create-test`) | `generators/chainsaw_test.yaml.tmpl` | `chainsaw_generator.go` |
