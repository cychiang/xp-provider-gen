...
| 🧪 `test.yml` — Test | push/PR to `main`, `develop` | Unit tests with coverage (Codecov + artifact); when source files changed, the native-flavor e2e (`scripts/e2e-test.sh`) with its Docker-dependent Step E skipped (`E2E_SKIP_DOCKER=1`) — Step E itself runs in `e2e-native-full.yml` |
| 🧭 `e2e-native-full.yml` — Native E2E (full) & Upgrade Sim | daily; `workflow_dispatch`; PRs touching the generator/template/version surfaces this exercises | The same native e2e with Step E included (no `E2E_SKIP_DOCKER`) — the generated provider's own uptest+chainsaw suite against a real kind cluster — plus `make upgrade-sim` |
