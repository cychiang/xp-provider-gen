...
|---|---|---|
| 🧹 `lint.yml` — Lint | push/PR to `main`, `develop` | golangci-lint at the version pinned here (Renovate bumps only this file — the same version must be set by hand in three other places, see [docs/development.md](../docs/development.md#requirements)), a `gofmt` check, and a `go mod tidy` check |
| 🧪 `test.yml` — Test | push/PR to `main`, `develop` | Unit tests with coverage (Codecov + artifact); when source files changed, the native-flavor e2e (`scripts/e2e-test.sh`) with its Docker-dependent step 12 skipped (`E2E_SKIP_DOCKER=1`) — step 12 itself runs in `e2e-native-full.yml` |
| 🧭 `e2e-native-full.yml` — Native E2E (full) & Upgrade Sim | daily; `workflow_dispatch`; PRs touching the generator/template/version surfaces this exercises | The same native e2e with step 12 included (no `E2E_SKIP_DOCKER`) — the generated provider's own uptest+chainsaw suite against a real kind cluster — plus `make upgrade-sim` |
...
```bash
# Re-run the native e2e (full, with step 12) + upgrade simulation on demand
