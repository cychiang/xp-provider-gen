...
# adopt also stamps the generator version into PROJECT (confirmed by Task 2's
# manual run), so the diff is exactly these two files — not just provider.go.
ADOPT_DIFF="$(git diff --name-only | sort)"
EXPECTED_DIFF="$(printf 'PROJECT\nconfig/provider.go\n')"
[ "$ADOPT_DIFF" = "$EXPECTED_DIFF" ] ||
  fail "adopt's diff was '$ADOPT_DIFF', expected exactly PROJECT and config/provider.go"
