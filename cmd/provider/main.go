...
import (
	...
	"github.com/test/gitignore-test/apis"
	providercontroller "github.com/test/gitignore-test/internal/controller"
	"github.com/test/gitignore-test/internal/version"
)

func main() {
	var (
		app            = kingpin.New(filepath.Base(os.Args[0]), "gitignore-test support for Crossplane.").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("gitignore-test"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(ratelimiter.LimitRESTConfig(cfg, *maxReconcileRate), ctrl.Options{
		...
		LeaderElectionID:           "crossplane-leader-election-gitignore-test",
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add gitignore-test APIs to scheme")
	...
	if *enableChangeLogs {
		...
		clo := controller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("gitignore-test:%s", version.Version))),
		}
		o.ChangeLogOptions = &clo
	}

	kingpin.FatalIfError(providercontroller.Setup(mgr, o), "Cannot setup gitignore-test controllers")
